...
<body>
  ...
  async convertToBuffer(markdownContent, options = {}) {
    ...
    try {
      ...
      // 性能优化：禁用不必要的资源加载，但保留字体加载
      await page.setRequestInterception(true);
      page.on('request', (req) => {
        const resourceType = req.resourceType();
        const url = req.url();
        
        // 允许Google Fonts和其他重要字体资源
        if (url.includes('fonts.googleapis.com') || url.includes('fonts.gstatic.com') || resourceType === 'font') {
          req.continue();
        }
      ...
      await page.setContent(htmlDocument, { 
        ...
        timeout: this.config.timeout?.pageLoad || 8000 // 增加超时时间以等待字体
      });
